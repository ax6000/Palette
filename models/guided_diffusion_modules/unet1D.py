...
class ResBlock(EmbedBlock):
    ...
    def __init__(
        ...
        self.out_layers = nn.Sequential(
            normalization(self.out_channel).half(),
            SiLU(),
        ...
        # print("res init",self.out_channel,channels)
...
class AttentionBlock(nn.Module):
    ...
    def __init__(
        ...
        self.use_checkpoint = use_checkpoint
        self.norm = normalization(channels).half()
...
class QKVAttention(nn.Module):
    ...
    def forward(self, qkv):
        ...
        weight = torch.softmax(weight.half(), dim=-1).type(torch.float16)
...
class UNet(nn.Module):
    ...
    def forward(self, x, gammas):
        ...
        :return: an [N x C x ...] Tensor of outputs.
        """
        isnan = lambda x: torch.any(torch.isnan(x))
        # print("a",x.shape)
        ...
        # print("h",h.shape,"emb",emb.shape,emb.dtype)
        # print("unet -3",h.is_leaf,isnan(x),isnan(h),isnan(gammas),isnan(emb),emb.dtype,x.dtype)
        for module in self.input_blocks:
            h = module(h, emb)
            hs.append(h)
        h = self.middle_block(h, emb)
        # print("unet -2",isnan(h),h.is_leaf)
        for module in self.output_blocks:
            h = torch.cat([h, hs.pop()], dim=1)
            h = module(h, emb)
        # print("unet -1",isnan(h),h.is_leaf)
        h = h.type(x.dtype)
        # print("unet out",isnan(self.out(h)),isnan(h),h.is_leaf)
        return self.out(h)
