...
#         super(self).__init__()
#         # in (n_batch, 256)
#         # out (n_batch, 2) SBP, DBP
#         # conv
#         self.conv = nn.Conv1d(1,32,9,1)
...
class Network(BaseNetwork):
    ...
    def set_new_noise_schedule(self,ddim_num_steps=100,ddim_eta=0.,verbose=True, ddim_discretize="uniform", device=torch.device('cuda'), phase='train'):
        ...
        # p2 weighting
        self.register_buffer('snr', to_torch(1. / (1 - gammas) - 1))
        self.register_buffer('p2_k', to_torch(1.))
        self.register_buffer('p2_gamma', to_torch(0.5))
        # self.register_buffer('p2_gamma', 1)
        self.loss_fn.set_snr(self.snr)
    ...
    def forward(self, y_0, y_cond=None, mask=None, noise=None):
        ...
        # loss
        weight  = extract(1 / (self.p2_k + self.snr)**self.p2_gamma, t, x_shape=(1,))
        weight = weight[..., None,None].expand(-1,-1,noise.shape[-1])
        loss = weight * (noise_hat - noise) ** 2
        loss = loss.mean()
        # print(weight.shape,noise.shape,loss.shape)
        return loss
    ...
    def ddim_sampling(self, cond, shape,
        ...
        for i, step in enumerate(time_range):
            ...
            outs = self.p_sample_ddim(img, cond, ts, index=index, use_original_steps=ddim_use_original_steps,
                                      ...
                                      unconditional_conditioning=unconditional_conditioning)
            img, pred_x0 = outs
            # print(img.shape,pred_x0.shape,torch.mean(img))
    ...
    def p_sample_ddim(self, x, c, t, index, repeat_noise=False, use_original_steps=False, quantize_denoised=False,
        ...
        noise_level = extract(self.gammas, t, x_shape=(1,)).to(t.device)
        e_t = self.denoise_fn(torch.cat([c,x], dim=1), noise_level)
        # e_t = self.denoise_fn(x, t, c)
        alphas = self.ddim_alphas
        alphas_prev =  self.ddim_alphas_prev
        sqrt_one_minus_alphas = self.ddim_sqrt_one_minus_alphas
        sigmas = self.ddim_sigmas
        # print("alphas",alphas[index],"alphas_prev",alphas_prev[index])
